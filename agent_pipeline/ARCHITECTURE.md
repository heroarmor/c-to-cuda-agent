...
The compiler optimize moves are in the same state: `compiler_moves.py`'s planning and its mechanical accept/reject gate are tested against a fake toolchain (`test_compiler_moves.py`), but no real nvcc/PPCG candidate has been measured yet. First GPU check for them: `python3 agent_pipeline/run_pipeline.py benchmark/easy/dense-linalg/saxpy.c --backend ppcg --optimizer compiler` and confirm the run produces real per-candidate timings in `optimize_result.json` and (if a flag wins) a sane `nvcc_flags.txt` next to the exported `.cu`.
