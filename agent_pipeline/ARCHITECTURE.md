...
- **Generic vs. dataset-specific**: `run_pipeline.py` takes a plain file path and knows nothing about "the dataset" — `agent_pipeline/` stays purely the agent's own code. `evaluation/run_dataset.py` is the thin, separate driver (under `evaluation/`, not nested inside `agent_pipeline/`) that loops it over every benchmark in `benchmark/` — the same separation `cuda/bench.sh` and the other `evaluation/` drivers keep.
...
An iteration's optimize slot can be taken by a deterministic, orchestrator-side **compiler move** instead of the `cuda-optimize` agent — the implementation of `polyhedral/DESIGN.md`'s "Compiler backend in the *optimize* stage" (Phase 2.5), in `agent_pipeline/compiler_moves.py`. Three moves, dispatched in this order while untried profile-guided candidates remain:

- **`library`** — cuBLAS substitution, first priority because it's algorithmic-tier (the playbook's own ordering: algorithm > hardware tuning > flags). When the program's `scop_targets.json` entry carries `blas` metadata (a square row-major GEMM hot function), `polyhedral/cublas_to_cu.py` regenerates the `.cu` from the original C source with that function's body replaced by a `cublasSgemm`/`cublasDgemm` call (row-major handled by the swapped-operand column-major identity). On acceptance, `-lcublas` joins the persisted flag set so later verify/profile rebuilds keep linking. Tried at most once per run; ppcg-backend only (like `retile`, it regenerates from C). The mechanical gate still decides — at small sizes the handle + copies can lose to the C baseline, and that's a legitimate rejection.
- **`flags`** — an nvcc flag search, backend-agnostic. A small candidate set (≤3 per iteration) is proposed from the parsed `ncu_summary` (`--maxrregcount` when occupancy is low / registers high, `-Xptxas -dlcm=cg` then `-dscm=cs` on memory stalls, `-use_fast_math` only when the `.cu` actually calls transcendentals, `-Xptxas -O3 --allow-expensive-optimizations` in the no-signal generic ladder), then each candidate is compiled, run, diffed against `baseline_output.txt`, and timed — mechanically, inside this one optimize slot (best-of-K, so a sweep never spreads across iterations and reads as stagnation to the patience stop). A winner must beat the iteration's profiled time by `MIN_DELTA`; it's persisted to **`nvcc_flags.txt`**, which the verify/profile skills include in every later nvcc invocation and which travels with best-snapshots and the exported `.cu` (the tuned time isn't reproducible from the `.cu` alone).
- **`retile`** — PPCG re-tiling, only when the `.cu` came from the ppcg backend. Re-runs `polyhedral/ppcg_to_cu.py` on the *original C source* (PPCG consumes C, not CUDA) with a new schedule candidate chosen from the measured signal — a `--sizes` string (smaller blocks for low occupancy, bigger tiles for memory-bound) or a schedule option (`--tile-size=64`, `--no-shared-memory`, `--unroll-gpu-tile`) — behind the same mechanical accept/reject gate. A losing candidate leaves the workdir's `.cu` untouched. A hybrid-backend `.cu` (Phase 3's PPCG-partial + LLM-stitched glue) is deliberately *not* retile-eligible: regenerating from the C source would reproduce only the partial translation and clobber the agent's stitching — only the flags move applies there.

One directed escalation: when the parsed profile is **launch-overhead shaped** (launch count ≥ 1000 or the summary says so — multigrid's first e2e run measured ~98k launches with 64% of wall time spent launching), the fix is a structural host rewrite no compiler knob can make, so a hybrid-mode LLM optimize move carries an explicit `CUDA_Graph_Capture` hint appended to its prompt (the technique is catalogued in `cuda-bottleneck-playbook`); detection is mechanical, the risky code transform stays with the agent, and the next verify gates it.

Both moves write the same schema-validated `optimize_result.json` the agent writes, so the loop, patience stop, and best-version tracking are move-agnostic; each iteration's `pipeline_result.json` entry records which optimizer took the slot (`optimize_move`). In `hybrid` mode the `cuda-optimize` agent takes over once no compiler candidate remains; in `compiler` mode the run instead stops with exit_reason `optimizer_exhausted` (a normal exit, not a hard error) — combined with `--backend ppcg` that makes the generate + optimize stages zero-token on the affine subset (the verify/profile stages still run as opencode agents, so the loop isn't model-free end-to-end). The moves live in the orchestrator deliberately: `cuda-optimize` has `bash: deny`, and the mechanical gate (real compile/run/diff/time, not model judgment) is the same "objective signal over agent self-report" preference used everywhere else here — it's also what makes a numerics-changing candidate like `-use_fast_math` safe to even try, since an output mismatch just rejects that candidate. `compiler_moves.py`'s planning half is pure and covered by `test_compiler_moves.py` (which fakes the toolchain to exercise the accept/reject gate end-to-end without a GPU).
...
- `cuda-verification-procedure/compare_outputs.py` — tolerance-aware line/number diff between two program outputs (the same golden-output diffing philosophy `cuda/build_run.sh` uses to score the tracked conversions, reimplemented standalone so `agent_pipeline` stays self-contained).
...
Mechanical tools were deliberately used instead of asking the LLM to compute statistics or eyeball a diff — consistent with how the suite's own drivers (`cuda/build_run.sh`, `evaluation/`) do tolerance-based output comparison externally rather than trusting embedded self-checks.
